...
class FlowDiffuser_Sp8Cascade_Adjustable(nn.Module):
    def __init__(self, args):
        super().__init__()
        print('\n ---------- model: FlowDiffuser w 1/8 scale Cascade Refinement Stage, flexible iters ver. ---------- \n')
        ...
        self.update = SKUpdate(self.args, hidden_dim=c_dim) # this same update block is used in both the cascade refinement stage & diffusion stage.
        ...
        if self.diffusion:
            ...
            print(' -- denoise steps: %d \n' % self.ddim_n)
            print(' -- recurrent iterations (conditional-rdd): %d \n' % self.recurr_itrs)
            print(' -- recurrent iterations (cascade refinement): %d \n' % self.args.iters_cascade)
            self.n_sc = 0.1
    ...
    def _prepare_targets(self, flow_gt):
        """
        Forward diffusion step during training, for 1/8 GT flow
        """
    ...
    def _q_sample(self, x_start, t, noise=None):
        """
        Forward diffusion step, for 1/8 GT flow
        """
        if noise is None:
    ...
    def _ddim_sample(self, feat_shape, net, inp, coords0, coords1_init, clip_denoised=True):
        """
        The entire DDIM reverse denoising process.
        """
    ...
    def _model_predictions(self, x, t, net, inp8, coords0, coords1, i_ddim, pred_last=None, t_next=None):
        """
        A single denoising step, using multiple iterative refinements (6 in the original code)
        """
        ...
        for ii in range(self.recurr_itrs):
            ...
            with autocast(enabled=self.args.mixed_precision):
                ...
                first_step = False if itr != 0 else True
                dfm_params = [t_ii, self.update, ii, 0]
                #dfm_params = [t_ii, self.update_dfm, ii, 0]
                net, delta_flow = self.update_dfm(net, inp8, corr, flow, itr, first_step=first_step, dfm_params=dfm_params)
                up_mask = self.um8(net)
    ...
    def forward(self, image1, image2, test_mode=False, iters=None, flow_gt=None, flow_init=None):
        ...
        if self.diffusion:
            ...
            # 1/8 scale cascade refinement. I guess this is the subnet or something.
            for iter in range(self.args.iters_cascade):
                coords1 = coords1.detach()
                
                corr = self.corr_fn(coords1)
                flow = coords1 - coords0
                with autocast(enabled=self.args.mixed_precision):
                    net, delta_flow = self.update(net, inp8, corr, flow, iter, sp4=True)
                    up_mask = self.um8(net)
